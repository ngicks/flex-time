...
func ReplaceTimeToken(input string) (string, error) {
	...
	for len(input) > 0 {
		prefix, token, input, isToken, err = nextToken2(input)
		if err != nil {
			return "", err
		}
		output += prefix
...
func nextToken2(input string) (prefix string, found string, suffix string, isToken bool, err error) {
	for i := 0; i < len(input); i++ {
		...
		case '.':
			if strings.HasPrefix(input[i:], ".S") ||
				...
				repeated := getRepeatOf2(input[i+1:], input[i+1:i+2])
...
func getRepeatOf2(input string, target string) string {
	for i := 0; i < len(input); i++ {
